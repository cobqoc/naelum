...
### 기능 구현 현황
- **조리 단계 타이머 UX — 단계마다 버튼 1개로 통합** — 완료 (2026-05-22, develop 푸시)
  - **문제**: 시간 점 2개+ 단계에 `⏱4분`·`⏱2분`·`⏱단계 타이머` 3버튼 노출(잡다) + 직접 타이머가 조리순서 최상단에만 있어 스크롤 중 접근 불편
  - **수정**: 단계별 타이머를 **단계마다 `⏱ 타이머` 1개**로 통합 — 클릭 시 `CustomTimerSetup` 모달이 그 단계 본문 시간으로 prefill(0·1·여러 개)되어 열림. 별도 `N분` 퀵버튼·`단계 타이머` 버튼·상단 전역 버튼 모두 제거
  - 모든 단계에 버튼 → 스크롤하며 어느 단계서나 타이머 사용. 단계에서 시작한 타이머는 패널에 "N단계"로 표시(stepNumber 전달)
