...
### 기능 구현 현황
- **조리 단계 타이머 UX 정리 — 시간 있는 단계 + 하단 바** — 완료 (2026-05-22, develop 푸시)
  - **문제**: 시간 점 2개+ 단계에 `⏱4분`·`⏱2분`·`⏱단계 타이머` 3버튼 노출(잡다). 직접 타이머가 조리순서 최상단에만 있어 스크롤 중 접근 불편. (중간 시도 "모든 단계에 버튼 1개"는 시간 없는 단계에도 군더더기)
  - **최종**: 단계별 `N분` 퀵버튼·`단계 타이머` 버튼·상단 전역 버튼 전부 제거. 대신:
    - **본문에 시간이 있는 단계**에만 `⏱ 타이머` 1개 — 클릭 시 `CustomTimerSetup`이 그 단계 시간으로 prefill 되어 열림 (콩나물 "4분, 2분 뒤집기" 류). 시간 없는 단계엔 버튼 없음
    - **하단 고정 액션 바**(장보기·피드백 옆)에 `⏱ 타이머` — 스크롤해도 항상 보이는 엄지존. 어느 단계서나 즉흥 타이머. 빈 폼으로 열림
  - 단계에서 시작한 타이머는 패널에 "N단계"로 표시(stepNumber 전달)
...
### 재료 DB
- **prod: 740개 approved** / **dev: 907개** (`ingredients_master`, 2026-05-19 카테고리 3차 감사 완료 — other 0개, 분포: grain 147·veggie 137·seafood 117·fruit 80·meat 70·seasoning 61·spice 21·condiment 14·dairy 39·bakery 21·beverage 20·snack 13)
