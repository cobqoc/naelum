...
> 5개 god-file 전부 분해 완료(아래 표). 공통 규약: 순수함수 → `lib/`+vitest,
> 순수 표현 → `_components/`(상태·ref·race·async 핸들러는 부모 불변, 콜백만),
> JSX byte-identical, 추출 전 회귀 안전망 선작성(기존 강커버 시 갭만 보강).
> 분해 안전망이 선존 RLS 데이터유실 버그까지 발견·수정([[project-recipe-children-rls-fix]]).
