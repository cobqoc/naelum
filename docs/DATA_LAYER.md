...
- `[RATCHET]` **select-star** — `select('*')` 사용처 수. **상한 55곳** (시작 67 → 55).
...
> **후속 서버 병목 개선**: 직렬 round-trip 병렬화(preferences·users/me·search getUser) + delivery/restaurants `.limit(200)` + 공개 엔드포인트 캐싱(trending·delivery `unstable_cache`, ingredients `Cache-Control` — 실측 ~24-29ms→~2-3ms) + 검색 fridge-match 2회→union 1회. (PR #246)
