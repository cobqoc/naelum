...
- **코드+DB read-only 감사 → 가드레일 + 죽은 레거시 정리** (2026-06-03)
  ...
  - **Supabase write `.error` 미체크 전수 수정**: 10곳(`.insert/.update/.delete` 후 에러 무시 → Supabase는 RLS거부/제약위반 시 throw 안 하고 `{error}` 반환 → 침묵 데이터유실). 성격별 처리: **사용자/어드민 명시 작업은 표면화(500)** — admin substitute-suggestions delete·push/subscribe(구독해제)·search/history(검색기록 삭제). **분석/cron은 로그(흐름 안 깸)** — recommendations/track(3)·cart/share 만료토큰 revoke·send-expiry 만료구독 정리. (send-expiry notifications·search/route insert는 이미 처리됨). 검증: lint·build·풀 e2e 450.
...
- **비로그인 라이브 전수 점검 → i18n/UX 6건 + 추천 배지 오판 버그 근본 수정** (2026-06-03, develop bc176ca·a0792fc·09a09bd)
  ...
  - **추천 카드 배지 "바로 가능" 오판 (a0792fc)**: 냉장고 쌀 1개인데 7재료 레시피가 "바로 가능"(녹색). 원인 = RecipeCard 배지는 `recipe.missingIngredientNames.length`로 판정하는데 추천 API(`/api/recommendations` type=ingredients)는 `missingCount`(숫자)만 반환·이름 배열 누락 → undefined→[]→0. `attachFridgeMatch`(전체·검색)는 이름배열을 만들지만 추천 route와 갈라져 있었음. → `matchV2.buildMatchNameArrays` 추출해 양쪽 공용, route 응답에 이름배열 추가. 라이브(쌀 1개): 쌀필요죽=바로가능, 밥필요=1개부족, 카레볶음밥=7개부족.
  - **구조적 뿌리 제거 (09a09bd)** — 증상 fix 후 남은 두 뿌리: ① **경로 이중화** → `matchV2.assembleRecipeMatchFields` 단일 출처로 추출, fridgeMatch·route 공유(불변식 `missingCount===missingIngredientNames.length`). ② **RecipeCard "모름→초록(바로 가능)" footgun**(배지를 이름배열 length로 판정해 데이터 없음을 "다 있음"으로 추정) → `lib/recommendations/fridgeBadge.getFridgeBadge` 순수함수(명시 missingCount만 신뢰, 불완전 시 null=배지없음). vitest는 environment:node라 컴포넌트 테스트 불가 → 순수함수로 빼야 테스트 가능. **DB 아닌 코드 문제 확인**: 매칭 *계산*은 정확, 서버 *직렬화*가 이름배열 누락.
  - **상세 "N/M 보유" 배지가 카드와 불일치 (라이브 추가 발견)**: 같은 레시피(쌀 1개)에 카드 "6개 부족"인데 상세는 "0/7 보유"로 모순. 상세 배지가 `ownedCount`(정확보유만)를 써서 밥(쌀로 충족)을 안 셈 → 카드는 충족으로 셈. → `matchV2.countMatched` 순수함수(kind 기반 충족/없는/전체) 추출, `useRecipeFridgeMatch`에 `coveredCount`·`coverageStatus` 추가(정확보유 `ownedCount`는 cart "보유 제외"용으로 유지). 상세 배지 = `coveredCount`/total → **"1/7 보유"**(1충족+6부족=7, 카드와 일관, 색도 빨강→주황). 단일 기준 불변식 테스트로 카드(assembleRecipeMatchFields)와 같은 숫자 보장. (홈 "재료가 더 필요한 레시피 1개"는 *레시피 개수*라 별개 — 정상)
  - **검증**: lint 0 · build · vitest 332(신규 fridgeBadge footgun 가드 + assemble 불변식 포함) · 풀 e2e 450 green. CLAUDE.md 정정(cart=DB·로그인 필요) + i18n 규칙 3개(데모사전↔demoItems.ts 동기화·법적페이지 한글전용·API 한글에러 raw 노출 금지). 메모리 [[project_logged_out_i18n_fixes_20260603]]·[[project_ingredient_match_v2_shipped_broken]]. **교훈: "데이터 없음"의 기본값을 "긍정(초록/바로가능)"으로 두지 말 것.**
...
- **"만들어봤어요" 기록 중심 리프레임 + 따라만들기 흐름 연결** (2026-06-02)
  ...
  - **모델**(스키마 변경 0): "만들어봤어요"=`cooking_sessions`(만든 사실, 별점 없어도 기록·"N명 만듦" 카운트), 별점/후기=`recipe_posts` 리뷰(선택, 피드·평점). 두 카운트 분리(🍳 만든 수 · ⭐ 평점 수) — 정직.
