...
describe('buildMatchNameArrays — RecipeCard 이름 배열 (배지 단일 출처)', () => {
  ...
  it('countMatched — 상세 배지·카드 단일 기준 (밥=충족 1, 카드 missingCount와 일치)', () => {
    const summary = matchRecipe(ingredients, userIdSet, graph);
    const counts = countMatched(ingredients, summary.results);
    // 쌀만 보유 → 밥=충족(1), 마늘·양배추·대패삼겹살=없음(3). 상세 "1/4 보유"가 카드 "3개 부족"과 일관.
    expect(counts).toEqual({ matchedCount: 1, missingCount: 3, totalCount: 4 });
    const fields = assembleRecipeMatchFields(ingredients, summary, userIdToName);
    expect(counts.matchedCount).toBe(fields.matchedCount);
    expect(counts.missingCount).toBe(fields.missingCount);
