...
const KEYWORD_MAP: Array<[string, string]> = [
  ...
  ['과카몰리', '🥑'], ['살사', '🍅'], ['후무스', '🥜'],
  ['잼', '🍓'],
  // ── 2026-05-17 칩 게이트 후속 보강 ──────────────────────────────
  // dev ingredients_master 감사(scripts/audit-emoji-coverage.ts)로
  // 찾은, 흔한데 precise 누락된 재료. 정확·고유 이모지가 있는 것만
  // (폴백 노이즈 재도입 아님). 배열 끝=최저 우선순위라 기존 매핑 불간섭.
  ['고춧가루', '🌶️'],          // '고추' 키워드가 받침차이(춧≠추)로 못 잡음
  ['단호박', '🎃'], ['호박', '🎃'], // 애호박은 EXACT '🥒' 우선
  ['무우', '🥕'], ['단무지', '🥕'],
  ['파슬리', '🌿'],
  ['어묵', '🍢'],
  ['떡', '🍡'],                 // 떡국떡·가래떡 등 (떡갈비는 '갈비' 우선)
  ['메밀', '🌾'],
  ['유자', '🍋'],
];
