...
interface ReviewComposerModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void; // 성공 시 피드 새로고침
}

// "만들어봤어요" — 별점(필수) + 후기(선택) → POST /posts (rating). 인증 리뷰 작성.
export default function ReviewComposerModal({ recipeId, isOpen, onClose, onSuccess }: ReviewComposerModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`/api/recipes/${recipeId}/posts`, {
        ...
        body: JSON.stringify({ rating, content: review.trim() || null }),
      });
      if (!res.ok) { const d = await res.json(); throw new Error(d.error || t.recipe.reviewSaveFailed); }
      onSuccess();
      onClose();
    } catch (e) {
      toast.error(e instanceof Error ? e.message : t.recipe.reviewSaveError);
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm overflow-y-auto p-4" role="dialog" aria-modal="true">
      <div ref={panelRef} className="bg-background-secondary rounded-2xl p-6 max-w-md w-full my-8 border border-white/10">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold text-text-primary">{t.posts.reviewCta}</h3>
        ...
        <div className="mb-6">
          <label className="block text-sm font-bold text-text-primary mb-3">{t.recipe.reviewRatingLabel}</label>
          <div className="flex justify-center gap-2">
            {[1, 2, 3, 4, 5].map((star) => (
              <button key={star} onClick={() => setRating(star)} disabled={submitting}
                className="text-4xl transition-transform hover:scale-110 active:scale-95 disabled:opacity-50">
                {star <= rating ? '⭐' : '☆'}
              </button>
            ))}
          </div>
          <p className="text-center text-text-muted text-sm mt-2">{rating}{t.recipe.reviewRatingScoreSuffix}</p>
        </div>

        <div className="mb-6">
          <label className="block text-sm font-bold text-text-primary mb-2">{t.recipe.reviewLabel}</label>
          <InputBoxWrapper className="!rounded-xl !px-4 !py-3 !min-h-[100px] !items-start">
            <textarea value={review} onChange={(e) => setReview(e.target.value)} disabled={submitting}
              ...
              style={INPUT_INNER_STYLE} rows={4} maxLength={500} />
          </InputBoxWrapper>
          <p className="text-xs text-text-muted text-right mt-1">{review.length}/500</p>
        </div>
        ...
        <button onClick={handleSubmit} disabled={submitting}
          ...
          ) : <span>{t.recipe.reviewSubmitCreate}</span>}
        </button>
      </div>
    </div>
  );
